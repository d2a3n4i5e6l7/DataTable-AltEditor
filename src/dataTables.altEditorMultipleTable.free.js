...
(function (factory) {
	...
	(function ($, window, document, undefined) {
		...
		var altEditor = function (dt, opts) {
			...
			//console.log('.altEditor' + (_instance));
		...
		$.extend(
			altEditor.prototype,
			{
				...
				_setup: function () {
					var that = this;
					var dt = this.s.dt;
					var modal = `<div class="modal fade" id="altEditor-modal${that.s.namespace}" tabindex="-1" role="dialog">` +
						...
						`<input type="submit" form="altEditor-form${that.s.namespace}" class="btn btn-primary"></input>` +
						'</div>' +
						'</div>' +
						'</div>' +
						'</div>';
					...
					if (dt.button('edit:name')) {
						...
						$(document).on('click', `#editRowBtn${that.s.namespace}`, function (e) {
					...
					if (dt.button('add:name')) {
						...
						$(document).on('click', `#addRowBtn${that.s.namespace}`, function (e) {
							if (that._inputValidation()) {
								e.preventDefault();
								e.stopPropagation();
								that._addRowData();
				...
				_openEditModal: function () {
					...
					data += `<form name="altEditor-form${that.s.namespace}" role="form">`;
					...
					for (var j in columnDefs) {
						...
						if (columnDefs[j].type.indexOf("hidden") >= 0) {
							...
							data += "<label style='font-weight: bold' for='" + columnDefs[j].title + "'>" + columnDefs[j].title + ":</label></div>";
							...
							if (columnDefs[j].type.indexOf("select") >= 0) {
								...
								data += `<select class='form-control${that.s.namespace}` + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + columnDefs[j].name + "' name='" + columnDefs[j].title + "' " + (columnDefs[j].multiple ? 'multiple' : '') + ">" + options
									+ "</select>";
							}
					...
					console.log($('#altEditor-modal.altEditor0'));

					$(`#altEditor-modal${that.s.namespace}`).on('show.bs.modal', function () {
						var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Cerrar</button>' +
							`<button type="button" data-content="remove" class="btn btn-primary" id="editRowBtn${that.s.namespace}">Editar</button>`;
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-title').html('Editar Registro');
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-body').html(data);
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-footer').html(btns);
					});

					$(`#altEditor-modal${that.s.namespace}`).modal('show');
					$('#altEditor-modal input[0]').focus();

					// enable select 2 items
					for (var j in columnDefs) {
						if (columnDefs[j].select2) {
							$(`#altEditor-modal${that.s.namespace}`).find("select#" + columnDefs[j].name).select2(columnDefs[j].select2);
						}
					}
				},
				...
				_editRowData: function () {
					...
					$(`form[name="altEditor-form${that.s.namespace}"] *`).filter(':input').each(function (i) {
				...
				_openDeleteModal: function () {
					...
					for (var j in columnDefs) {
						if (columnDefs[j].type.indexOf("hidden") >= 0) {
							...
							data += "<div style='margin-left: initial;margin-right: initial;' class='form-group row col-md-12'><label style='font-weight:bold;' for='"
								+ that._quoteattr(columnDefs[j].title)
								+ "'>"
								+ columnDefs[j].title
					...
					$(`#altEditor-modal${that.s.namespace}`).on('show.bs.modal', function () {
						var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Cerrar</button>' +
							`<button type="button"  data-content="remove" class="btn btn-danger" id="deleteRowBtn${that.s.namespace}">Eliminar</button>`;
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-title').html('Eliminar Registro');
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-body').html(data);
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-footer').html(btns);
					});

					$(`#altEditor-modal${that.s.namespace}`).modal('show');
					$(`#altEditor-modal${that.s.namespace} input[0]`).focus();
				},
				...
				_openAddModal: function () {
					...
					for (var j in columnDefs) {
						if (columnDefs[j].type.indexOf("hidden") >= 0) {
							...
							data += "<div style='margin-left: initial;margin-right: initial;' class='form-group row'><div class='col-sm-3 col-md-3 col-lg-3 text-right' style='padding-top:4px;'><label  style='font-weight:bold;' for='"
								+ columnDefs[j].title
								+ "'>"
								+ columnDefs[j].title
							...
							if (columnDefs[j].type.indexOf("select") >= 0) {
								...
								data += `<select class='form-control${that.s.namespace}` + (columnDefs[j].select2 ? ' select2' : '') + "' id='" + that._quoteattr(columnDefs[j].name) + "' name='" + that._quoteattr(columnDefs[j].title) + "' " + (columnDefs[j].multiple ? 'multiple' : '') + ">" + options
									+ "</select>";
							}
							data += "</div><div style='clear:both;'></div></div>";
						}
					...
					$(`#altEditor-modal${that.s.namespace}`).on('show.bs.modal', function () {
						var btns = '<button type="button" data-content="remove" class="btn btn-default" data-dismiss="modal">Cerrar</button>' +
							`<button type="button"  data-content="remove" class="btn btn-primary" id="addRowBtn${that.s.namespace}">Agregar</button>`;
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-title').html(
							'Agregar Registro');
						$(`#altEditor-modal${that.s.namespace}`).find('.modal-body')
							.html(data);
						$(`#altEditor-modal${that.s.namespace}`)
							.find('.modal-footer')
							.html(btns);
					});
				...
				_deleteRowCallback: function (response, status, more) {
					$(`#altEditor-modal${that.s.namespace} .modal-body .alert`).remove();

					var message = '<div class="alert alert-success" role="alert">' +
						'<strong>Success!</strong>' +
						'</div>';
					$(`#altEditor-modal${that.s.namespace} .modal-body`).append(message);
					...
					$(`div#altEditor-modal${that.s.namespace}`).find(`button#addRowBtn${that.s.namespace}`).prop('disabled', true);
					$(`div#altEditor-modal${that.s.namespace}`).find(`button#editRowBtn${that.s.namespace}`).prop('disabled', true);
					$(`div#altEditor-modal${that.s.namespace}`).find(`button#deleteRowBtn${that.s.namespace}`).prop('disabled', true);
				},
